...
function kaliTerusRekursif (angka) {   
  var kali = 0;     
  while (angka.toString().length > 1) { // "654", length = 3
    angka = angka.toString().split (""); // ["6", "5", "4"]
    angka = multiply (angka); // multiply (["6", "5", "4"])
    kali++;
  }
  return angka;
}

function multiply (num) { // ["6", "5", "4"]
  if (num.length === 1 ) { // BASE CASE
    return Number (num);
  } else {
    return Number(num.splice(0, 1)) * multiply (num.slice(0));
  }
}
