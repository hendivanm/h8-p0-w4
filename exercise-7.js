...
function digitPerkalianMinimum (angka) {
    var number = 0;
    var numberMinimum = 0;
    for (var i = 0; i <= angka; i++) { // JIKA < angka, MAKA TEST CASE TERAKHIR MENJADI 0
        if (angka % i === 0) {
            number = i.toString().length + (angka / i).toString().length;
            // UNTUK number
        }
        if (numberMinimum === 0 || number < numberMinimum) {
